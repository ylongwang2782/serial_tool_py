...
class SerialDebugger:
    ...
    def start(self):
        try:
            ...
            self.log_to_file(
                "Info", f"Connected to {self.port} at {self.baudrate} baudrate"
            )
        ...
        while self.running:
            if self.serial_connection.in_waiting > 0:
                ...
                while "a5ffcc" in buffer:
                    ...
                    pin_num = int(frame[14:16], 16)
                    devive_status = frame[16:20].upper()
                    frame_data = frame[20 : end_index - 2].upper()
                    spaced_frame_data = " ".join(
                        frame_data[i : i + 2] for i in range(0, len(frame_data), 2)
                    )
    ...
    def update_treeview(self, slot_num, frame_type, hex_data):
        ...
        if slot_num in self.treeview_rows:
            self.treeview.item(
                self.treeview_rows[slot_num],
                values=(slot_num, frame_type_text, hex_data),
            )
        else:
            row_id = self.treeview.insert(
                "", slot_num, values=(slot_num, frame_type_text, hex_data)
            )
            self.treeview_rows[slot_num] = row_id

        self.log_to_file(
            slot_num, f"Type: {frame_type_text}, Data: {hex_data}", timestamp
        )

    def update_data_treeview(
        self,
        slot_num,
        frame_type,
        pin_num,
        ...
        if slot_num in self.data_treeview_rows:
            self.data_treeview.item(
                self.data_treeview_rows[slot_num],
                values=(
                    timestamp,
                    slot_num,
                    frame_type_text,
            ...
            row_id = self.data_treeview.insert(
                "",
                slot_num,
                values=(
                    timestamp,
...
class SerialDebuggerGUI:
    def __init__(self, root):
        ...
        self.refresh_button = tk.Button(
            root, text="Refresh", command=self.refresh_ports
        )
        ...
        self.baudrate_combobox.grid(row=1, column=1, columnspan=1, sticky="ew")
        ...
        self.treeview = ttk.Treeview(
            root, columns=("Slot", "Frame Type", "Data"), show="headings"
        )
        ...
        self.data_treeview = ttk.Treeview(
            root,
            columns=("Timestamp", "Slot", "Frame Type", "PinNum", "Status", "Data"),
            show="headings",
        )
    ...
    def toggle_connection(self):
        if self.debugger and self.debugger.running:
            ...
            self.debugger = SerialDebugger(
                port, baudrate, self.treeview, self.data_treeview
            )
            self.debugger.start()
